            ...
            {faqs.map((item, idx) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <div className="text-base font-medium text-slate-900">{item.q}</div>
                    <ChevronRight
                      className={`shrink-0 text-slate-500 transition-transform ${open ? "rotate-90" : "rotate-0"}`}
                      size={18}
                    />
                  </button>
                  {open && (
                    <div className="mt-3 max-w-3xl text-sm leading-relaxed text-slate-600">{item.a}</div>
                  )}
                </div>
              );
            })}
