...
if not SUPABASE_URL:
    log.warning("SUPABASE_URL not set — Supabase and JWT auth will be unavailable.")
...
async def lifespan(app: FastAPI):
    ...
    if os.path.exists(model_path):
        try:
            ...
            log.warning(f"Model load failed: {e}. Continuing without model.")
    else:
    ...
    if SUPABASE_URL:
        try:
            ...
            log.warning(f"Supabase/JWKS init failed: {e}. Auth endpoints will return 503.")
    else:
        log.warning("Supabase not configured. Auth endpoints unavailable.")
...
if __name__ == "__main__":
    uvicorn.run("app:app", host="0.0.0.0", port=int(os.getenv("PORT", 8001)), reload=False)